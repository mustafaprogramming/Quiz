...
let answerQuizTypescript:string[]=["Static Typing","Dynamic","Readonly","Function myFunc(): void","Both 1 and 2","To describe the shape of an object","A type that can be one of several types","Type guard","--strict","function  foo(param?: string)","tsc example.ts","let variable: string | null;","To indicate a function always throws an exception or never returns.","{ [key: string]: any; }","npm install -g typescript"];
let answerQuizIQTest:string[]=["13","YES","Hand","Even","15","1.45","6","1","sea","36","Some Pets are playful","4","Barcelona","9","Peru"];
let answerQuizGeneralKnowleadge:string[]=["Red","Peregrine Falcon","Angel Falls, Venezuela","Mars","Carbon","Honey","The Sumerians","Around 50,000","Ottawa","To house the tombs of pharaohs","Brown","13","5","He","Pack"];
//type of question
type questionObject={
    question:string;
    options:string[];
...
// Functions
function checkAnswer(quizAnswer:string[],userAnswer:string[]): number{
    let Score=0;
...
async function askingQuestions(questions:questionObject[],answerArray:string[]): Promise<number>{
